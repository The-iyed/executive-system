                          ...
                          <div className="grid grid-cols-1 gap-3">
                            {execSummaryAttachments.map((att) => (
                              <div key={att.id} className="group flex items-center gap-3 px-4 py-3 bg-white border border-[#E5E7EB] rounded-xl hover:border-[#92400E]/30 hover:shadow-sm transition-all duration-200">
                                {att.file_type?.toLowerCase() === 'pdf' ? <img src={pdfIcon} alt="pdf" className="h-10 object-contain flex-shrink-0" /> : <div className="w-10 h-10 bg-[#F3F4F6] rounded-lg flex items-center justify-center text-xs font-bold text-[#DC2626] flex-shrink-0">{att.file_type?.toUpperCase() || ''}</div>}
                    ...
                    <div className="flex flex-col gap-3">
                      {(meeting?.attachments || []).filter((a) => a.is_presentation && !deletedAttachmentIds.includes(a.id)).map((att) => (
                        <div key={att.id} className="group flex items-center gap-4 px-5 py-4 bg-white border border-[#E5E7EB] rounded-xl hover:border-[#048F86]/40 hover:shadow-[0_2px_12px_rgba(4,143,134,0.08)] transition-all duration-200">
                          {att.file_type?.toLowerCase() === 'pdf' ? <img src={pdfIcon} alt="pdf" className="h-11 object-contain flex-shrink-0" /> : <div className="w-11 h-11 bg-[#F3F4F6] rounded-lg flex items-center justify-center text-xs font-bold text-[#DC2626] flex-shrink-0">{att.file_type?.toUpperCase() || ''}</div>}
                          <div className="flex flex-col items-end min-w-0 flex-1">
                      ...
                      {newPresentationAttachments.map((file, idx) => (
                        <div key={`new-pres-${idx}`} className="flex items-center gap-3 px-5 py-4 bg-[#F0FDF9] border border-dashed border-[#048F86]/50 rounded-xl">
                          {file.name.toLowerCase().endsWith('pdf') ? <img src={pdfIcon} alt="pdf" className="h-10 object-contain" /> : <div className="w-10 h-10 bg-[#F3F4F6] rounded-lg flex items-center justify-center text-xs font-bold text-[#DC2626]">{file.name.split('.').pop()?.toUpperCase() || 'FILE'}</div>}
                              ...
                              <div className="flex flex-wrap gap-1.5">
                                {isLocal ? (
                                  <>
                                    ...
                                    <label className="flex items-center gap-1.5 rounded-full bg-[#ECFDF5] px-2.5 py-1 cursor-pointer" onClick={(e) => e.stopPropagation()}>
                                      <input type="checkbox" checked={accessChecked} disabled={!canEdit} onChange={(e) => updateLocalInvitee(row.id, 'access_permission', e.target.checked)} className="w-3 h-3 rounded border-gray-300 text-[#048F86] focus:ring-[#048F86] disabled:opacity-60" />
                                      <span className="text-xs text-[#059669]">صلاحية</span>
                                    </label>
                                  </>
                                ) : (
                                  <>
                                ...
                                {row.justification && (
                                  <TooltipProvider delayDuration={200}>
                                    <Tooltip>
                                      ...
                                      <TooltipContent side="top" className="max-w-[350px] text-right whitespace-pre-wrap">
                                        {row.justification}
                                      </TooltipContent>
                                    </Tooltip>
                                  </TooltipProvider>
                          ...
                          <div key={row.id || index} className={`group relative border rounded-xl ring-2 ring-[#048F86]/20 transition-all duration-200 ${isConsultant ? 'bg-[#F0FDF9] border-[#048F86]/25' : 'bg-white border-[#E5E7EB]'}`} style={{ boxShadow: '0 1px 3px rgba(16,24,40,0.04)' }}>
                            <div className="flex flex-col gap-3 p-4">
                              ...
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                <label className="flex items-center gap-1.5 rounded-full bg-[#ECFDF5] px-2.5 py-1 cursor-pointer" onClick={(e) => e.stopPropagation()}>
                                  <input type="checkbox" checked={accessChecked} onChange={(e) => updateMinisterAttendee(index, 'access_permission', e.target.checked ? 'FULL' : 'READ_ONLY')} className="w-3 h-3 rounded border-gray-300 text-[#048F86] focus:ring-[#048F86]" />
                                  <span className="text-xs text-[#059669]">صلاحية الاطلاع</span>
                                </label>
                                <label className="flex items-center gap-1.5 rounded-full bg-[#F3F4F6] px-2.5 py-1 cursor-pointer" onClick={(e) => e.stopPropagation()}>
                                  <input type="checkbox" checked={!!row.is_required} onChange={(e) => updateMinisterAttendee(index, 'is_required', e.target.checked)} className="w-3 h-3 rounded border-gray-300 text-[#048F86] focus:ring-[#048F86]" />
                                  <span className="text-xs text-[#6B7280]">مطلوب</span>
                                </label>
                                <label className="flex items-center gap-1.5 rounded-full bg-[#F0FDF9] border border-[#048F86]/20 px-2.5 py-1 cursor-pointer" onClick={(e) => e.stopPropagation()}>
                                  <input type="checkbox" checked={!!row.is_consultant} onChange={(e) => updateMinisterAttendee(index, 'is_consultant', e.target.checked)} className="w-3 h-3 rounded border-gray-300 text-[#048F86] focus:ring-[#048F86]" />
                                  <span className="text-xs text-[#048F86]">مستشار</span>
                                </label>
                              </div>
