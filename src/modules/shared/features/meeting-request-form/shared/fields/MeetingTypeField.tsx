...
import { MEETING_TYPE_OPTIONS } from "../../enums";
import { useIsFieldEditable } from "../hooks/EditableFieldsContext";

interface Props { disabled?: boolean }

export function MeetingTypeField({ disabled }: Props) {
  const { control, formState: { errors } } = useFormContext();
  const editable = useIsFieldEditable("meeting_type");
  const isDisabled = disabled || !editable;
  return (
    <FormField label="نوع الاجتماع" name="meeting_type" required errors={errors} colSpan={4}>
